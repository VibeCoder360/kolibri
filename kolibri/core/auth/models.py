...
class FacilityUser(AbstractBaseUser, KolibriBaseUserMixin, AbstractFacilityDataModel):
    ...
    # printed on the user's credential card. Learners are assigned one automatically;
    # coaches, admins, and superusers can opt in via the ID cards API. The token is
    # globally unique so a single token resolves to exactly one user. Like
    # `picture_password`, it is intentionally
...
class Role(AbstractFacilityDataModel):
    ...
    def save(self, *args, **kwargs):
        ...
        with transaction.atomic():
            ...
            result = super().save(*args, **kwargs)
            # Picture passwords are learner-only, so promotion to a role clears
            # them. QR login tokens are deliberately kept: coaches and admins
            # may hold a QR token too, so a promoted learner keeps their card.
            ...
            if (
                ...
                if not are_picture_passwords_exhausted(user.dataset_id):
                    try:
                        ...
                        pass
            # Auto-assignment remains learner-only; staff opt in explicitly
            # via the assign/rotate API actions.
            if (
